...
from ..model.attention import FeatureAttention
...
class embed_LSTM_ITE(nn.Module):
    ...
    def __init__(
        ...
        self.attention_X = (
            FeatureAttention(
                input_dim=self.x_input_dim, hidden_dim=self.x_input_dim // 2
            )
            if attention
            else None
        )
    ...
    def _out_gate(self, x: torch.Tensor, h_prev: torch.Tensor) -> torch.Tensor:
