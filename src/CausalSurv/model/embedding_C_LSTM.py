...
class embed_LSTM(nn.Module):
    ...
    def __init__(
        ...
        self.linear_forget_Wxf = nn.Linear(
            self.x_embed_dim, self.hidden_length, bias=True
        )
        self.linear_forget_Whf = nn.Linear(
            self.hidden_length, self.hidden_length, bias=False
        )
        self.linear_forget_Wdf = nn.Linear(1, self.hidden_length, bias=False)
        self.linear_forget_Wpf = nn.Linear(
            self.p_embed_dim, self.hidden_length, bias=False
        )
        self.sigmoid_forget = nn.Sigmoid()

        # Input gate
        self.linear_input_Wxi = nn.Linear(
            self.x_embed_dim, self.hidden_length, bias=True
        )
        self.linear_input_Whi = nn.Linear(
            self.hidden_length, self.hidden_length, bias=False
        )
        self.sigmoid_input = nn.Sigmoid()

        # Cell memory (candidate g)
        self.linear_cell_Wxg = nn.Linear(
            self.x_embed_dim, self.hidden_length, bias=True
        )
        self.linear_cell_Whg = nn.Linear(
            self.hidden_length, self.hidden_length, bias=False
        )
        ...
        self.linear_out_Who = nn.Linear(
            self.hidden_length, self.hidden_length, bias=False
        )
        self.linear_out_Wpo = nn.Linear(
            self.p_embed_dim, self.hidden_length, bias=False
        )
    ...
    def _input_gate(self, x: torch.Tensor, h_prev: torch.Tensor) -> torch.Tensor:
        ...
        """
        i = self.sigmoid_input(self.linear_input_Wxi(x) + self.linear_input_Whi(h_prev))
        return i

    def _cell_memory_gate(
        self,
        i: torch.Tensor,
        f: torch.Tensor,
        x: torch.Tensor,
        ...
        """
        g = self.activation_cell(self.linear_cell_Wxg(x) + self.linear_cell_Whg(h_prev))
        return g * i + f * c_prev

    def _out_gate(
        self, x: torch.Tensor, h_prev: torch.Tensor, p: torch.Tensor
    ) -> torch.Tensor:
...
if __name__ == "__main__":
    ...
    print("Output sa shape:", sa.shape)  # Expected: (batch_size, output_sa_length)
    print(
        "Final hidden state shape:", h_0.shape
    )  # Expected: (batch_size, hidden_length)
    print(
        "Final cell state shape:", c_0.shape
    )  # Expected: (batch_size, hidden_length)  # Expected: (batch_size, hidden_length)
    print("Final p state shape:", p_0.shape)  # Expected: (batch_size, p_embed_dim)
