...
class DynaSurv(L.LightningModule):
    ...
    def forward(self, XPd: torch.Tensor, X_static: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        for t in range(time_steps):
            ...
            hazards_out_logit.append(haz_t)
    ...
    def training_step(self, batch, batch_idx):
        XPd, X_static, interval_idx, treatment_idx, time, event = batch
        haz_pred_logit = self.forward(XPd, X_static)
        ...
        for line in range(interval_idx.size(1)):
            ...
            haz_pred_line = haz_pred_logit[:, line, :]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for line in range(interval_idx.size(1)):
            interval_idx_line = interval_idx[:, line]
            event_line = event[:, line]
            haz_pred_line_logit = haz_pred_logit[:, line, :]
        
            
            loss_line = self.loss_fn(haz_pred_line_logit, interval_idx_line, event_line)
            loss += loss_line

            y_hazard = self._predict_hazard_on_line(XPd[:, line, :], X_static, time[:, line])
    ...
    def predict_survival(self, x: torch.Tensor, x_static: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        haz_pred = torch.sigmoid(self.forward(x, x_static))
    ...
    def _predict_hazard_on_line(self, XPd_line: torch.Tensor, X_static: Tuple[torch.Tensor, torch.Tensor], eval_time: torch.Tensor) -> torch.Tensor:
        ...
        hazards_on_grid = torch.sigmoid(self.forward(XPd_line.unsqueeze(1), X_static).squeeze(1))
        hazards = hazards_on_grid.gather(1, interval_idx)

        return hazards
    
    def predict_hazard(self, x: torch.Tensor, x_static, eval_time: torch.Tensor) -> torch.Tensor:
        batch_size, time_steps, n_features = x.shape
        hazards_out = []

        for t in range(time_steps):
            haz_t = self._predict_hazard_on_line(x[:, t, :], x_static, eval_time[:, t])
...
if __name__ == "__main__":
    ...
    sa_pred = model(sample_XPd, sample_X_static)
