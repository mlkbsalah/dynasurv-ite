...
def train_test_split_survival_data(df:pd.DataFrame, test_size:float=0.2, random_state: int | None = None,
                                   time_col = 'Y_onset_to_death', event_col = 'Y_death') -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    
    df_train, df_test = train_test_split(df, test_size=test_size, random_state=random_state)
    if df_train[time_col].max() < df_test[time_col].max():
        max_train_time = df_train[time_col].max()
        df_test = df_test[df_test[time_col] <= max_train_time]

    y_train = Surv.from_dataframe(event_col, time_col, df_train)
    y_test = Surv.from_dataframe(event_col, time_col, df_test)
...
def RepeatedKFold_survival(df:pd.DataFrame, n_splits=5, n_repeats=1, random_state: int | None = None,
                           time_col = 'Y_onset_to_death', event_col = 'Y_death'):
...
def intergrated_brier_score(model, y_train, y_test, x_test, time_col = 'Y_onset_to_death') -> float:
    lower, upper = np.percentile(y_test[time_col], [10, 90])
