...
class ESMEOnlineDataModule(L.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        print(
            f"Loaded {len(self.data)} rows, {self.data['usubjid'].nunique()} patients."
        )
    ...
    def setup(self, stage: str | None = None):
        ...
        P_encoded = pd.get_dummies(
            self.data[P_cols + ["usubjid"]].astype(str), columns=P_cols, prefix="T"
        )
        ...
        ESME_Dataset = ESMEOnlineDataset(
            X_list,
            X_static,
            P_list,
            P_static,
...
if __name__ == "__main__":
    ...
    XPd, (x_static, p_static), interval_idx, treatment_indices, time, event, mask = (
        batch
    )
