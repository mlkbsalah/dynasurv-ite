...
class ESMEDataset(TorchData.Dataset):
    def __init__(self, X: torch.Tensor, X_static: torch.Tensor, P: torch.Tensor, P_static: torch.Tensor, d: torch.Tensor, time: torch.Tensor, event: torch.Tensor, n_lines: int, interval_bounds: torch.Tensor) -> None:
        """
        Args:
...
class ESMEDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: str | None = None):
        ...
        ESME_Dataset = ESMEDataset(X, X_static, P, P_static, d, time, event, self.n_lines, self.interval_bounds)
    ...
    def train_dataloader(self):
        return TorchData.DataLoader(self.train_dataset, batch_size=self.train_batch_size, shuffle=True, num_workers=1, persistent_workers=False)
    
    def val_dataloader(self):
        return TorchData.DataLoader(self.val_dataset, batch_size=len(self.val_dataset), shuffle=False, num_workers=1, persistent_workers=False)
