...
class PropensityOverlapModel:
    ...
    def fit(
        ...
        """Fit models and obtain factual propensities out of fold on training data."""
        self.line_results = {}
        for line in range(self.n_lines):
            arms = np.asarray(eligible_arms_per_line.get(line, []), dtype=int)
            if len(arms) < 2:
                continue
            observed = mask[:, line].bool().cpu().numpy()
            labels = treatment_idx[:, line].cpu().numpy()
            keep = observed & np.isin(labels, arms)
            y = labels[keep]
            if len(y) == 0 or len(np.unique(y)) < 2:
                continue

            features = self.features_from_tensors(X, X_static, P, d, line)[keep]
            min_class_count = int(np.bincount(np.searchsorted(arms, y)).min())
            n_splits = min(self.n_splits, min_class_count)
            if n_splits < 2:
                continue

            oof = np.empty((len(y), len(arms)), dtype=float)
            ...
            for train, test in splitter.split(features, y):
                ...
                oof[test] = estimator.predict_proba(features[test])
    ...
    def predict_mask(
        ...
        result = torch.zeros(batch_size, self.n_lines, n_treatments, dtype=torch.bool)
