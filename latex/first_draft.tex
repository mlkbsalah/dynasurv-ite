...
\begin{figure}
    ...
    \caption{Model architecture of the counterfactual survival prediction model \textcolor{red}{First draft figure requires legend and probably color coding of the blocks to distinguish prediction blocks from model ones}}
\end{figure}
...
\begin{itemize}
    \item \textbf{Cox PH} (per line, fitted independently). A standard Cox proportional hazards model fitted at each line $k$ on the patient state $X_k$ and treatment indicator $A_k$. This is the conventional clinical baseline.
    \item \textbf{DeepSurv}. A multilayer perceptron applied per-line to the same flat feature vector $[X_k, A_k]$, trained with the logistic hazard loss. This isolates the value of nonlinear modeling without any sequential or causal structure.
    \item \textbf{TARNet (no LSTM)}. A treatment-specific-head architecture identical to ours but without the sequential encoder, the auxiliary progression head, and the IPM regularizer. This isolates the value of the head structure alone.
    \item \textbf{Ours, $-$ progression head}. Full model without the auxiliary progression head and without the $\rho^P$ feedback into the encoder.
    \item \textbf{Ours, $-$ IPM}. Full model with the representation balancing term disabled ($\alpha = 0$).
    \item \textbf{Ours, $-$ LSTM}. Full model with the LSTM encoder replaced by an MLP applied to the most recent state $X_k$ and previous treatment $A_{k-1}$.
    \item \textbf{Ours (full)}. The complete architecture described in Section~\ref{sec:model-overview}.
\end{itemize}

All models are trained on the same training split, evaluated on the same held-out test set, and metrics are reported with bootstrap 95\% confidence intervals over \textcolor{red}{$B$ = ?} resamples of the test set.

\paragraph{Headline performance.} Table~\ref{tab:factual-headline} reports the C-index and IBS pooled across all treatment lines. \textcolor{red}{[One sentence stating the headline finding: e.g., the full model achieves a C-index of X (95\% CI [a, b]) and IBS of Y, outperforming the strongest baseline (Z) by W. Each ablation degrades performance, with the largest drop coming from removing the [LSTM / IPM / progression head].]}

\begin{table}[h]
\centering
\begin{tabular}{lcc}
\hline
\textbf{Model} & \textbf{C-index $\uparrow$} & \textbf{IBS $\downarrow$} \\
\hline
Cox PH                  & \textcolor{red}{.} & \textcolor{red}{.} \\
DeepSurv                & \textcolor{red}{.} & \textcolor{red}{.} \\
TARNet (no LSTM)        & \textcolor{red}{.} & \textcolor{red}{.} \\
Ours, $-$ progression   & \textcolor{red}{.} & \textcolor{red}{.} \\
Ours, $-$ IPM           & \textcolor{red}{.} & \textcolor{red}{.} \\
Ours, $-$ LSTM          & \textcolor{red}{.} & \textcolor{red}{.} \\
\textbf{Ours (full)}    & \textcolor{red}{.} & \textcolor{red}{.} \\
\hline
\end{tabular}
\caption{Factual prediction quality, pooled across treatment lines. Values are mean (95\% CI) over bootstrap resamples of the test set. Higher C-index and lower IBS indicate better performance.}
\label{tab:factual-headline}
\end{table}

\paragraph{Per-line breakdown.} Pooled metrics may obscure systematic differences across decision points. Figure~\ref{fig:factual-perline} reports the C-index and IBS as a function of treatment line for each model. \textcolor{red}{[One sentence on the trend: e.g., performance is highest at line 1 where the patient population is most homogeneous, and decreases at later lines as data become sparser and patient heterogeneity increases. The relative ordering of models is consistent across lines / changes at later lines, suggesting that ...].}
...
\caption{Factual prediction quality as a function of treatment line. Shaded regions are 95\% bootstrap confidence intervals. Lines beyond \textcolor{red}{$k = ?$} are pooled because per-line sample sizes drop below \textcolor{red}{$N$ = ?}.}
\label{fig:factual-perline}
\end{figure}

\paragraph{Calibration.} The C-index and IBS summarize discrimination and overall accuracy but do not directly assess whether predicted survival probabilities are well-aligned with observed frequencies. Figure~\ref{fig:calibration} shows calibration curves for the full model at evaluation horizons of \textcolor{red}{$t = ?$ months}. \textcolor{red}{[One sentence: e.g., the model is well-calibrated across the predicted-risk distribution at short horizons, with mild over-prediction in the highest-risk decile at longer horizons. Calibration is uniform / non-uniform across treatment arms.]}

\begin{figure}[h]
\centering
\textcolor{red}{[Figure: calibration curves at 2--3 evaluation horizons. x-axis: predicted survival probability (decile midpoints). y-axis: observed Kaplan--Meier survival probability in each decile. Diagonal reference line. One panel per horizon, or all on one plot with different markers. Optionally faceted by treatment arm to expose treatment-specific miscalibration.]}
\caption{Calibration of the full model at evaluation horizons \textcolor{red}{$t = ?$}. Each point is a decile of predicted survival; the y-coordinate is the observed Kaplan--Meier estimate within the decile. Perfect calibration corresponds to the diagonal.}
