...
\caption{Unadjusted 24-month overall survival from the start of line 1, by calendar year and treatment category. Error bars are 95\% confidence intervals; a (category, year) point is shown only when it has enough patients and enough follow-up to reach 24 months. Curves are crude: within a year the treatment groups are not exchangeable, so the gaps between categories reflect differences between the patients who receive each treatment, not treatment effects.}
\label{fig:km-year}
\end{figure}
...
\caption{12-month overall survival at lines 2--4 (columns a/d, b/e, c/f), by calendar period. Top row: year-granular trend by prior treatment at the start of the line --- black is all patients with histories mixed together, the naive trend; colored series hold a prior-treatment stratum fixed; dotted series are strata defined only from 2016 onward, once CDK4/6 inhibitors entered use. Error bars are 95\% confidence intervals, and a year is shown only with enough patients and follow-up to reach 12 months (2023 is dropped). Bottom row: the same milestone at coarser calendar eras, crude (black) versus direct standardization to a fixed prior ET/CT mix (grey); the gap between the two bars is the share of the crude trend explained by a change in who reaches the line rather than by better survival. All curves remain crude with respect to covariates other than prior treatment (performance status, metastatic burden, age).}
