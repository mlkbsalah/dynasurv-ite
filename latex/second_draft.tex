...
\begin{itemize}
    ...
    \item[ii.] A \emph{recommendation procedure} that only gives advice where the data can support it. Treatments are filtered by sample size, by how well-defined they are, by available follow-up, and by a patient-specific positivity check. When no treatment passes, the system abstains. The procedure also measures its own uncertainty with an ensemble of models trained from different random initializations: when the treatments cannot be separated, it returns the short list of equivalent options instead of a single choice. A sweep over the RMST horizon then checks that the ranking behind the advice does not depend on it.
...
\begin{figure}[h]
\centering
\includegraphics[width=.85\linewidth]{figs/treatment_category_repeats.pdf}
\caption{Repetition of treatment categories over the first four lines (patients with four lines; HR+HER2$-$). A repetition is a line given a category the patient already had: \emph{consecutive} when it is the immediately following line (e.g.\ ET, ET), \emph{non-consecutive} when it returns after a switch (e.g.\ ET, MONOCT, ET). (a) Patients by repetition kind. (b) Repetitions per category, split by kind. (c) All 15 possible sequence patterns over four lines, colored by kind; letters denote treatment categories in order of first appearance, so A A B C means lines 1 and 2 were the same treatment.}
\label{fig:repeats}
\end{figure}
...
Calendar time also hides a positivity problem. When all years are pooled and the propensity model ignores the start year, only 0.4\% of patients have a probability below 0.01 of receiving CDK4/6 inhibitors, and overlap looks excellent. When the start year is added to the propensity model as a check, this share rises to 36.5\%: many patients were treated before these drugs were in use, so their true probability was zero. The apparent overlap came from pooling periods in which the treatment did and did not exist. Adding calendar time to the model cannot fix this, because a probability that is truly zero cannot be adjusted for; the only remedy is to restrict the cohort to a period in which all treatments were available.

The same problem appears when we try to compare treatments on survival. Figure~\ref{fig:km-year} shows first-line survival by year for each treatment category, computed directly from the observed patients, without any adjustment. Within each category, survival barely changes over 15 years: calendar time changed which treatments were given far more than it changed the survival of patients on a given treatment. The gaps \emph{between} categories, however, are not treatment effects. Each treatment is given to different patients, and these differences in patients are mixed into the survival of each group.

The standard way to remove such differences is weighting. Each patient is weighted by the inverse of their probability of receiving the treatment they actually received, so that every treatment group comes to resemble the whole cohort. A second set of weights corrects for patients whose follow-up was cut short. We applied both over the whole study period, with 80 covariates including calendar year, and checked whether the treatment groups became balanced. Balance is measured by the standardized mean difference between groups, where values below 0.1 are usually considered acceptable \todo{[Austin, 2009]}. Weighting works for the treatments used throughout the period: the largest remaining difference is at most 0.07 for endocrine therapy alone, polychemotherapy, monochemotherapy, and \textsc{other}. It fails for the treatments tied to a period: the remaining difference is 0.42 for CDK4/6 inhibitors and 0.38 for anti-angiogenic combinations, and it sits on the year indicators (Figure~\ref{fig:adjustment}). Weighting can only give more weight to patients who exist, and no patient treated in 2010 received a drug introduced years later. This confirms, on survival data, the conclusion of the previous paragraph: a treatment that was not available cannot be adjusted for, and the cohort has to be restricted.

\begin{figure}[h]
\centering
\includegraphics[width=.8\linewidth]{figs/km_line1_24mo_trend.pdf}
\caption{Unadjusted 24-month overall survival from the start of line 1, by calendar year and treatment category. Error bars are 95\% confidence intervals; a (category, year) point is shown only when it has enough patients and enough follow-up to reach 24 months. Within a year, the treatment groups are not comparable, so the gaps between categories reflect differences between the patients who receive each treatment, not treatment effects.}
\label{fig:km-year}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=.85\linewidth]{figs/km_line1_adjustment_diagnostics.pdf}
\caption{Diagnostics of the inverse-probability weighting at line 1, one row per treatment category. (a) Covariate balance, measured as the maximum standardized mean difference (SMD) against the pooled remainder across all recorded covariates, before (orange) and after (blue) weighting; the dotted line marks the conventional balance threshold of 0.1. (b) Effective sample size: raw patient count (grey) versus the effective count after weighting (blue) --- weighting buys comparability by leaning on fewer effective patients. (c) Share of each category's patients by calendar year of line-1 onset; a row concentrated in one block of years has no counterpart in other eras, which weighting cannot fix. Balance fails for the categories that were only available during part of the study period.}
\label{fig:adjustment}
\end{figure}

\paragraph{Design decision.} We restrict the cohort to patients whose \emph{first} line starts in 2018 or later. By 2018, CDK4/6 inhibitors were already in routine use and anti-angiogenic combinations had disappeared, so the set of available treatments is stable over the window. Selecting on the first line, rather than on each line separately, keeps every patient's trajectory complete. The restriction removes the availability problem: over the whole period, calendar time strongly predicts the treatment received (macro-AUC 0.63--0.68 across lines 1--4 for a propensity model using calendar time alone), whereas within the window the same model does no better than chance (AUC 0.50--0.54 at every line). Treatment choice still depends on the patient's clinical state, but no longer on when the patient started. Gradual changes in practice may remain inside the window; they are ordinary confounding, and we keep the time since the start of the window as a covariate to account for them.

\subsection{Finding 3: Later Lines Depend on Earlier Treatments}

Finding~1 showed that earlier treatments shape the choice of the next one. They also shape who reaches a later line and how long these patients survive. We show this with survival at lines 2 to 4 over calendar time, where the change in earlier treatments is largest.

Without any adjustment, 12-month overall survival rose by 4.3, 10.9, and 11.9 points at lines 2, 3, and 4 between 2008--2011 and 2020--2024, whereas first-line survival did not improve (Finding~2). This does not mean that later-line treatments became better. The patients reaching these lines had received different treatments before: the share of line-4 patients previously treated with CDK4/6 inhibitors went from zero before 2016 to about 70\% by 2020--2024. When we compare periods with the same mix of previous treatments (direct standardization), the gains shrink to 1.1, 5.1, and 5.5 points (Figure~\ref{fig:later-lines}), and what remains happened between 2008--2011 and 2012--2015, before CDK4/6 inhibitors arrived. At least half of the apparent improvement therefore comes from a change in treatment history, not from better survival for a given history.

Earlier treatments also decide who reaches a later line at all. Reaching line $k$ means having progressed and survived through all earlier lines. After CDK4/6 inhibitors arrived, fewer patients reached later lines: within four years of starting line 1, the share reaching line 2 fell from about 70\% to 59\%, and the share reaching line 4 from about 33\% to 26\%. Those who did reach them took about as long as before (median 11.9 to 13.1 months to line 2, 27.8 to 28.0 months to line 4). The population at a later line is therefore not fixed: it results from the treatments given at earlier lines.

\begin{figure}[h]
\centering
\includegraphics[width=.7\linewidth]{figs/km_later_lines_trend.pdf}
\caption{12-month overall survival at lines 2--4 (columns a/d, b/e, c/f), by calendar period. Top row: year-granular trend by prior treatment at the start of the line --- black is all patients, without adjustment; colored series hold a prior-treatment stratum fixed; dotted series are strata defined only from 2016 onward, once CDK4/6 inhibitors entered use. Error bars are 95\% confidence intervals, and a year is shown only with enough patients and follow-up to reach 12 months (2023 is dropped). Bottom row: the same milestone at coarser calendar eras, unadjusted (black) versus direct standardization to a fixed prior ET/CT mix (grey); the gap between the two bars is the share of the unadjusted trend explained by a change in who reaches the line rather than by better survival. No curve is adjusted for covariates other than prior treatment (performance status, metastatic burden, age).}
\label{fig:later-lines}
\end{figure}

\paragraph{Design decision.} Two consequences follow. First, previous treatments affect survival at later lines, not only the choice of the next treatment, so they act as confounders. The model therefore conditions on the full history $H_k$ (Section~\ref{sec:formulation}), and a representation of the current state alone would not be enough. Second, the patients at line $k$ are themselves a consequence of earlier treatments. The estimand at line $k$ is defined among patients who reach that line (Section~\ref{sec:estimand}), and a change in earlier practice changes this population. This is one reason we recommend one line at a time rather than claiming an optimal sequence, and why a model trained on all lines together must handle populations that differ from line to line (Section~\ref{sec:calibration}).
...
Before the static-data join, the restricted cohort contains 6,256 patients and 13,257 line records, all with a first line starting in 2018 or later. After the join and removal of one exact duplicate patient-line row, the model uses 6,191 patients and 13,116 records. Protocol v2 reserves the 1,977 patients entering in 2021 or later for temporal testing. The 4,214 earlier patients are split into 3,371 training patients and 843 development-validation patients with fixed validation seed 0. Scaling, the survival grid and recommendation support models are fitted only on training patients. Early stopping and hyperparameter selection use development validation, not the temporal test set. The numerical experiments below predate this separation and must be rerun; changing the code cannot undo previous inspection of the historical hold-out.

The test set is, by construction, the part of the cohort with the shortest follow-up. We therefore choose the RMST horizon of each line based on the follow-up available in the test set: $\tau = 24$, 18, 12, and 12 months for lines 1 to 4. At these horizons, the survival status at $\tau$ is known, through a death before $\tau$ or a follow-up reaching it, for 62\%, 50\%, 50\%, and 45\% of the test records at lines 1 to 4, and the longest observed follow-up is 37.5, 32.0, 29.0, and 19.9 months. Longer horizons would require extrapolating beyond the observed follow-up; Section~\ref{sec:rec-horizon} checks how much the recommendation depends on this choice.
...
\paragraph{Recurrent update.} Let $h_{j-1}$ and $c_{j-1}$ be the hidden and memory states after line $j-1$, initialized from static features as described below. The cell computes
\begin{align}
...
\paragraph{Static features (protocol v2).} Let $z$ concatenate the 16 static patient features and 17 static pretreatment-history features, and let $z_P$ denote the latter alone. We initialize $h_0=\tanh(W_hz+b_h)$, $c_0=\tanh(W_cz+b_c)$ and $e(A_0)=\tanh(W_pz_P+b_p)$ with learned linear projections. These states carry baseline information into the first decision and subsequent history; the current treatment is not an input to them. Historical checkpoints retain their zero-state behavior when loaded for inspection and require retraining. The timing and imputation provenance of static inputs must still be verified upstream.
...
\paragraph{Calibration status and optional recalibration.} The line-specific temperatures $\gamma_k$ and biases $\beta_k$ are trained jointly with the survival likelihood. Selecting a \texttt{bestCALIB} checkpoint chooses an epoch using the validation calibration gap; it does not perform post-hoc recalibration. The training and recommendation pipelines do not call the experimental post-hoc calibrator. That optional procedure refits only temperatures and biases using survival bins fixed from a reference model, with all other weights frozen. If adopted, it must be fitted within development data, separately recorded, and evaluated on the reserved test set. A monotone transformation of each interval hazard does not in general preserve the ordering of cumulative survival, RMST or the C-index. Both discrimination and treatment decisions must therefore be re-evaluated. Marginal factual calibration alone cannot establish calibration of counterfactual curves.

The model gives a predicted survival curve for each patient and treatment; counterfactual calibration remains an empirical question. The next section explains how these curves are turned into a recommendation, and when the system should not give one.
...
\paragraph{Filter 4: patient-specific positivity.} Filters 1 to 3 define line-level eligibility. A separate multinomial logistic regression estimates assignment probabilities over \emph{all observed treatment classes}, including ineligible ones; eligibility is intersected with the propensity threshold only afterwards. Fitting only eligible classes would renormalize away lack of support for the entire eligible set. Inputs are pre-treatment clinical history, static patient features and previous treatments and elapsed durations; the current treatment is excluded. Training diagnostics use up to five stratified out-of-fold splits, with ordinary K-fold splits when a class has only one observation and a prior-only classifier when a training fold contains only one class. At inference, an eligible treatment also needs estimated propensity at least $0.01$. Missing propensity fits fail closed. This model is distinct from the adversarial propensity head. Its calibration and subgroup overlap require validation; effective sample size and low-propensity rates are diagnostics, not guarantees of positivity.

\subsection{Effect of the Filters}

On the current cohort, filters 1 to 3 reduce the choice to four treatments at line 1 (endocrine therapy alone, endocrine therapy combined with CDK4/6 inhibitors, monochemotherapy, and polychemotherapy), three at line 2 (polychemotherapy does not pass the global filters there), and two at lines 3 and 4 (monochemotherapy and polychemotherapy).

The patient-specific filter is sensitive to its threshold. In an earlier run with a threshold of 0.1, treatments that passed the global filters turned out to be unsupported for most individual patients: at line 1, monochemotherapy passed for only 6.6\% of test patients, polychemotherapy for 12.9\%, and endocrine therapy alone for 40.6\%, and the mean number of eligible treatments at line 1 dropped from 4.00 to 1.58. Such a filter no longer removes patients outside the support of the data; it removes most of the choice. With the current threshold of 0.01, evaluated with the ensemble of Section~\ref{sec:rec-uncertainty} (a treatment must pass for every member), the filter removes the chemotherapies from the patients least likely to receive them and little else. At line 1, endocrine therapy alone is eligible for 98.5\% of test patients, its combination with CDK4/6 inhibitors for 99.8\%, monochemotherapy for 83.8\%, and polychemotherapy for 78.8\%, for a mean of 3.61 eligible treatments out of 4. At line 2 the three treatments are eligible for 97.4\%, 99.5\%, and 99.9\% of patients (mean 2.97 of 3), and at lines 3 and 4 both chemotherapies are eligible for at least 98.9\% of patients. No test patient is left without an eligible treatment, and only 16 of the 3,454 patient-lines have a single one. In the earlier run, the factual metrics were almost unchanged by the filter (average C-index 0.651 and integrated Brier score 0.106, against 0.647 and 0.107 without it), as expected: the filter changes the advice, not the survival predictions.
...
\begin{enumerate}
    ...
    \end{equation}
    \item \emph{Decision.} The leader is recommended only if at least two treatments are eligible, it is alone in its equivalence set, and it wins in enough members, $p^{\text{best}}_\ell \geq \pi_{\min}$. Exact ties remain in the equivalence set even at zero margin.
\end{enumerate}
Each patient and line receives one of four outputs: \emph{confident} under the comparative rule above; \emph{undecided} when eligible alternatives cannot be separated; \emph{no support} when no treatment is eligible; or \emph{only supported option} when exactly one is eligible. The last status identifies that option but does not claim comparative superiority and does not populate the recommendation field.

\paragraph{Protocol migration.} The recommendation numbers below predate these corrections. The previously ``confident'' late-line singleton cases have no supported comparator. New runs use learned static-feature initial states, disjoint training/validation/test patients, training-only preprocessing and absolute assignment probabilities. Existing checkpoints cannot acquire these corrections retrospectively; the tables and figures must be regenerated after retraining.

We use $c = 1$, a margin $\delta = 1$ month, $\pi_{\min} = 0.7$, and $M = 40$ members, each taken at its epoch of smallest calibration gap (Section~\ref{sec:calibration}). Note that $p^{\text{best}}$ can only take the values $0, 1/M, \ldots, 1$: with $M = 40$, $\pi_{\min} = 0.7$ requires 28 members to agree, whereas a preliminary ensemble of three members required all of them.

\paragraph{Results.} Table~\ref{tab:rec-uncertainty} reports the three outputs on the test set, and Figure~\ref{fig:rec-mix} shows what is recommended and how far each patient is from a decision. At lines 1 and 2, the leader is endocrine therapy combined with a CDK4/6 inhibitor for 99.3\% of the patients, and it wins in 96\% and 98\% of the members on average. The rule nevertheless recommends it for only 71\% of the patients at line 1 and 44\% at line 2. The remaining patients are undecided, almost always between this combination and endocrine therapy alone: the members agree on the order, but the gain is within one month of RMST once the disagreement between members is subtracted. Figure~\ref{fig:rec-mix}(b) shows this directly. The undecided patients of lines 1 and 2 sit above the vote threshold and left of the margin, so it is the margin, not the vote, that withholds the recommendation; the effect of another choice of $\delta$ or $\pi_{\min}$ can be read from the same panel. At lines 3 and 4, the picture is different. Monochemotherapy and polychemotherapy cannot be separated for almost every patient: the leader changes from patient to patient (monochemotherapy for 61\% and 57\% of them), it wins in only 65\% and 69\% of the members on average, and the RMST gap between the two stays within one month. The system returns both treatments for 99\% of these patients. This is the expected behavior when the data do not contain a clear difference: the single-model $\arg\max$ would have split these patients between the two treatments, with no indication that the choice was fragile. Among the patients who receive a recommendation, it matches the treatment actually given in 69\% of cases at line 1 and 25\% at line 2. Agreement with practice is not a measure of quality, since the goal is to improve on practice, but the low value at line 2 shows that the model favors CDK4/6 inhibitors much more often than clinicians did at this line, where they gave them to 28\% of the patients (Figure~\ref{fig:rec-mix}(a)).

\begin{table}[h]
\centering
\begin{tabular}{crrrrrrr}
\toprule
\textbf{Line} & \textbf{Patients} & \textbf{Recommend} & \textbf{Undecided} & \textbf{Abstain} & \textbf{Mean set size} & \textbf{Leader vote share} & \textbf{Agreement} \\
\midrule
1 & 1{,}977 & 71.3\% & 28.7\% & 0.0\% & 1.34 & 0.96 & 69.1\% \\
2 & 841 & 44.1\% & 55.9\% & 0.0\% & 1.57 & 0.98 & 24.5\% \\
3 & 438 & 1.1\% & 98.9\% & 0.0\% & 1.99 & 0.65 & -- \\
4 & 198 & 1.0\% & 99.0\% & 0.0\% & 1.99 & 0.69 & -- \\
\bottomrule
\end{tabular}
\caption{Outputs of the ensemble recommendation on the test set ($M = 40$, $c = 1$, $\delta = 1$ month, $\pi_{\min} = 0.7$). Mean set size is the size of the equivalence set among patients with at least one eligible treatment, and leader vote share the mean of $p^{\text{best}}_\ell$ over the same patients. Agreement is the share of recommendations equal to the treatment actually given; it is not reported at lines 3 and 4, where fewer than ten patients receive a recommendation.}
\label{tab:rec-uncertainty}
\end{table}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figs/recommendation_mix.pdf}
\caption{Ensemble recommendation on the test set ($M = 40$). (a) Share of patients per line receiving each treatment from the clinicians, next to the leader of the ensemble. Colors identify the four treatments that can pass the filters; gray collects the other treatments given by clinicians. Solid segments are patients who receive a recommendation, hatched segments patients who are undecided, shown with their leader. (b) The decision rule, one point per patient: the horizontal axis is the pessimistic gap between the leader and its closest eligible rival, $\min_{a \neq \ell} (\bar{d}_a - c\, s^d_a)$, in months of RMST; the vertical axis is the leader's vote share $p^{\text{best}}_\ell$. Dashed lines are the margin $\delta = 1$ month and the threshold $\pi_{\min} = 0.7$; a recommendation is issued exactly when the point falls in the shaded quadrant. Points are colored by leader. Patients with a gap below $-3$ months are drawn at the left edge as triangles, and patients whose leader has no eligible rival at the right edge.}
\label{fig:rec-mix}
\end{figure}

\subsection{Robustness to the Horizon}
\label{sec:rec-horizon}

The horizons $\tau_k$ of Section~\ref{sec:cohort} were chosen from the follow-up available in the test set, not from clinical considerations, so the recommendation should not hinge on them. We therefore re-score the test set at every horizon from 3 to 36 months, in steps of 3 months, and compare with the reference horizon of each line. Only the integration limit of the RMST changes: the eligible sets, the members, and the thresholds are those of the reference, since the horizon also enters filter 3 and moving it would change which treatments can be compared at all, which is a different question. Figure~\ref{fig:rec-horizon} reports, per line and horizon, the leader mix, the share of patients whose leader is the same as at the reference horizon, and the overlap (Jaccard index) between the equivalence set at that horizon and the reference one.

At lines 1 and 2, the ranking does not depend on the horizon. From 6 months onward, the leader is the same as at the reference horizon for more than 99.5\% of the patients, and the equivalence set overlaps the reference one by at least 0.78 within six months of it. What moves with the horizon is the share of recommendations, from none at 3 months to 83\% at 36 months at line 1 (82\% at line 2). This is a consequence of the fixed margin: $\delta = 1$ month is a large fraction of a 3-month RMST and a small one of a 36-month RMST, so a longer horizon makes the same ranking easier to act on. Beyond the longest observed follow-up of the test set (gray band in Figure~\ref{fig:rec-horizon}), every RMST is an extrapolation of the model, and the increase in recommendations there should not be read as evidence. At lines 3 and 4, the equivalence set is always the pair of chemotherapies, but the leader within the pair changes with the horizon for 4\% to 16\% of the patients from 6 months onward, and for 17\% and 18\% of them at 3 months. The sweep therefore confirms the reading of Table~\ref{tab:rec-uncertainty}: the leaders of lines 1 and 2 are stable, and the rule only withholds the recommendation on the size of the gain, whereas the choice between the two chemotherapies at lines 3 and 4 is fragile, and the system is right to return both.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figs/recommendation_horizon.pdf}
\caption{Sensitivity of the recommendation to the RMST horizon, with the eligible sets and thresholds fixed at their reference values. (a) Leader mix per line at each horizon, with the colors of Figure~\ref{fig:rec-mix}; solid = recommended, hatched = undecided. (b) Agreement with the reference horizon of the line (dotted vertical line): share of patients with the same leader (solid line), mean Jaccard overlap between the equivalence sets (dashed line), and share of test records whose survival status is known at that horizon (dotted curve). The gray band starts at the longest observed follow-up of the line; beyond it, every RMST is an extrapolation.}
\label{fig:rec-horizon}
\end{figure}
...
The line-specific temperatures and biases of Section~\ref{sec:survival-heads} allow different hazard scales across lines, but this plot does not establish why the calibration error occurs. These parameters are fitted jointly, not post-hoc, in the reported pipeline (Section~\ref{sec:calibration}). \todo{Regenerate calibration curves and RMST gaps on the reserved test set after corrected training. If post-hoc recalibration is evaluated, also re-evaluate discrimination and treatment rankings; neither is guaranteed to be unchanged.}
...
\paragraph{Audit qualification.} The numerical sweep below is exploratory: its hold-out also served for early stopping and checkpoint-kind selection. Protocol v2 separates training, development validation and final temporal test patients, keeps every original patient's prefixes together, and fixes the checkpoint rule without optimizing reported test oracle regret. It reports exact expected factual Brier scores and uncensored latent-outcome checks, not training-cohort marginal IPCW scores. At the historical \texttt{bestCI} checkpoints, DynaSurv's mean pair PEHE is worse than per-arm Kaplan--Meier in nine of ten cells (reference: $1.761$ versus $1.705$ months), despite better reference policy regret ($0.336$ versus $0.516$ months). These results suggest some ranking benefit, not superior treatment-effect estimation. Corrected independent-test results are pending retraining.

\paragraph{Semi-synthetic design.} We keep the real patient covariates and treatment history and simulate the treatment of the line being scored, the potential survival time under every one of four arms, and censoring, following a Weibull model with arm-specific and covariate-dependent coefficients so that treatment effects vary between patients. Treatment assignment follows a softmax of the covariates, calibrated so its marginal arm mix matches the real data at every setting, which creates confounding of controlled strength. Because the data-generating process is known, the true survival curve and RMST of every patient under every arm can be computed exactly. Full design choices are in Appendix~\ref{app:semisynthetic}.

\paragraph{What breaks counterfactual estimates.} This benchmark first taught us which kind of confounding matters. When we increase confounding through \emph{observed} covariates, up to the point where 66\% of records are close to a positivity violation, outcome-regression baselines (Cox and DeepSurv with treatment as a covariate) remain unbiased. Only their variance increases. Methods that model the outcome given the covariates can handle observed confounding, as long as the model form is adequate. What biases them is \emph{hidden} confounding, that is, a variable affecting both treatment and outcome that the model does not see: the RMST bias of the Cox model grows steadily from $-0.02$ to $+0.73$ as the strength of the hidden variable increases from 0 to 4. We therefore evaluate our model along both axes; the current sweep has no Cox/DeepSurv baseline yet, so DynaSurv is scored against the oracle (true curves) and a covariate-free per-arm Kaplan--Meier only, over 3 replicates per cell (Appendix~\ref{app:semisynthetic:sweep-results}). Checkpoint choice matters as much as the confounding axes: pooled over all 30 runs, the real-data default (\texttt{val\_loss}) has the highest regret and PEHE of the four checkpoint kinds we score, against the \texttt{bestCI} checkpoint used below (Table~\ref{tab:counterfactual-sweep}, and Table~\ref{tab:semisynthetic:checkpoint} for the full comparison). As the observed-confounding strength $\gamma$ grows, PEHE and the average-effect bias roughly double, while the model's own policy regret stays flat within noise against a nearly unchanged random-policy headroom: arm \emph{ranking} degrades more slowly than the effect \emph{size} does, consistent with the baselines' variance-not-bias pattern above and with the collapsing overlap behind it (effective sample size on the last line falls from $0.24$ to $0.10$ over this range, Appendix Table~\ref{tab:semisynthetic:overlap}), though three replicates cannot rule out a small true bias on this axis. Hidden confounding is sharper: average-effect bias more than doubles from the reference (no hidden confounder feeding assignment) to the strongest setting we test, and regret rises with it, the pattern a model with no access to the confounder is expected to show.

\begin{table}[ht]
\centering\small
\caption{DynaSurv on the semi-synthetic sweep, \texttt{bestCI} checkpoint, mean $\pm$ std over 3 replicates (PEHE and $|$bias$|$ in months, pooled over lines and arm pairs; regret in months against the oracle; $h_r$ = random-policy regret, an upper bound on that cell's headroom). Reference cell: $\gamma=1$, hidden-confounder strength $0$, heterogeneity $1$. Full table, all axes, in Appendix~\ref{app:semisynthetic:sweep-results}.}
\label{tab:counterfactual-sweep}
\begin{tabular}{@{}llrrrrr@{}}
\toprule
Axis & Level & PEHE & $|$bias$|$ & Regret & Hit rate & $h_r$ \\
\midrule
$\gamma$ (observed) & 0 & $1.27\pm0.15$ & $0.34\pm0.15$ & $0.41\pm0.03$ & $0.57\pm0.05$ & 0.77 \\
$\gamma$ (observed) & 1.5 & $2.33\pm1.10$ & $1.49\pm1.03$ & $0.40\pm0.05$ & $0.62\pm0.02$ & 0.81 \\
$s$ (hidden) & 0.5 & $2.25\pm0.32$ & $1.49\pm0.24$ & $0.44\pm0.04$ & $0.59\pm0.02$ & 0.80 \\
$s$ (hidden) & 1 & $2.70\pm0.15$ & $2.14\pm0.16$ & $0.48\pm0.01$ & $0.57\pm0.01$ & 0.80 \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Ignorability and data quality.} The main limitation is the sequential ignorability assumption. Our cohort design and filters address positivity and consistency, but not unmeasured confounding. Age and other static covariates are now connected to the encoder, but this correction does not establish adequate adjustment or correct upstream imputation. Performance status is partly imputed in a way that underestimates the most impaired patients (Section~\ref{sec:data}). Hidden confounding can bias counterfactual estimates. A formal sensitivity analysis is needed to measure how strong an unmeasured confounder would have to be to change a recommendation.
