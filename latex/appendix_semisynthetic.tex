...
The model-side evaluation of this benchmark is not part of this appendix.
...
Both components are independent of the outcome and the arm, so censoring is non-informative
by construction. Real censoring probably is not, so this is the best case for
...
\begin{itemize}
    ...
    correct sign. Hidden confounding appears as bias, not as PEHE.
    ...
    random.
    \item \textbf{Factual accuracy.} IPCW concordance index and Brier score of the factual
    curve at $\tau_\ell$, with the censoring distribution of the training samples of the line
    --- the only part real data could also measure.
