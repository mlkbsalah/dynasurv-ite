...
| `recommendation_mix.py` | What the seed ensemble recommends on the temporal holdout and how sure it is, from a `reports/recommendations/<cohort>/<stamp>_<kind>_M<M>/` report written by `scripts/RecommendEnsemble.py`. Per line, the clinicians' arm mix next to the ensemble's leader mix (solid = confident recommendation, hatched = undecided), and the decision plane: one point per patient at (pessimistic gap to the runner-up in months, leader vote share) with the two thresholds drawn. Reads the newest `bestCALIB` report unless a directory is given. → `../latex/figs/recommendation_mix.pdf` |
| `recommendation_horizon.py` | Robustness of the recommendation to the RMST horizon, from a `..._horizons` sweep written by `scripts/HorizonRobustness.py` (action set and thresholds fixed, only the integration limit moves). Per line, the leader mix at each horizon (solid/hatched as above) and, against the configured reference horizon, the share of patients with the same leader, the Jaccard overlap of the equivalence sets and the share of holdout records observed to that horizon; grey band = beyond the last observed follow-up. → `../latex/figs/recommendation_horizon.pdf` |
